...
export default defineEventHandler(async (event) => {
  ...
  if (["JWT_SECRET", "G_CLIENT_ID", "G_CLIENT_SECRET"].find(k => !process.env[k])) {
    ...
    if (["/api/s", "/api/proxy", "/api/latest", "/api/mcp", "/api/analysis", "/api/xueqiu-my-stocks", "/api/news-etf-relation", "/api/intelligent-recommendation", "/api/data-sync", "/api/etf-abnormal-fluctuation", "/api/data-monitoring", "/api/performance-monitoring", "/api/user-feedback", "/api/etf-news-analysis"].every(p => !url.pathname.startsWith(p)))
