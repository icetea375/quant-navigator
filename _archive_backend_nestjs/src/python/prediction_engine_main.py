...
class PredictionEngineMain:
    ...
    def load_feature_data(self, trade_date: str, target_codes: List[str] = None) -> pd.DataFrame:
        """加载特征数据"""
        try:
            # 构建查询条件
            ...
            self.logger.info(f"Loaded feature data for {len(df)} stocks on {trade_date}")
            return df
    ...
    def load_text_data(self, trade_date: str, target_codes: List[str] = None) -> pd.DataFrame:
        """加载文本数据"""
        try:
            # 构建查询条件
    ...
    def train_models(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 保存到模型注册表
            self._save_to_model_registry(layer1_path, layer2_path, layer3_path, {
    ...
    def generate_predictions(self, trade_date: str, target_codes: List[str] = None) -> List[Dict[str, Any]]:
        """生成预测"""
        try:
            ...
            for _, row in feature_data.iterrows():
                try:
                    ...
                    # 准备特征
                    feature_columns = [
                        'return_zscore_60d', 'volume_zscore_60d',
                        'price_change_1d', 'price_change_5d', 'price_change_20d',
                        'pe_ttm', 'pb_ratio', 'market_cap', 'rsi_14', 'macd_signal',
                        'bollinger_position', 'turnover_rate', 'amplitude', 'volume_ratio'
                    ]
            ...
            self.logger.info(f"Generated {len(predictions)} predictions for {trade_date}")
            return predictions
    ...
    def _save_to_model_registry(self, layer1_path: str, layer2_path: str, layer3_path: str, performance: Dict[str, Any]) -> None:
        """保存模型到注册表"""
        try:
            cursor = self.db_connection.cursor()
            
            models = [
                ('PredictionEngine_LightGBM', 'LAYER1', layer1_path, performance['layer1']),
                ('PredictionEngine_FinBERT', 'LAYER2', layer2_path, performance['layer2']),
                ('PredictionEngine_Llama3', 'LAYER3', layer3_path, performance['layer3'])
            ]
            ...
            for model_name, model_type, model_path, perf in models:
                ...
                cursor.execute(insert_query, (
                    model_name, model_type, 'v1.0.0', model_path,
                    json.dumps(perf), True, 'prediction_engine'
                ))
...
def main():
    ...
    parser.add_argument('--mode', choices=['train', 'predict'], required=True, help='运行模式')
    ...
    try:
        ...
        if args.mode == 'train':
            ...
            # 生成预测
            predictions = engine.generate_predictions(args.trade_date, args.target_codes)
            print(f"生成了 {len(predictions)} 个预测结果")
            
            # 显示前5个预测结果
            for pred in predictions[:5]:
                print(f"{pred['target_code']}: {pred['p_final_prediction']:.4f} (置信度: {pred['confidence_score']:.2f})")
