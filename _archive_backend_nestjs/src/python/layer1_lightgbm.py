...
class Layer1LightGBM:
    ...
    def _engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'price_change_1d' in df.columns and 'price_change_5d' in df.columns:
                df['momentum_ratio'] = df['price_change_1d'] / (df['price_change_5d'] + 1e-8)
            
            if 'volume_ratio' in df.columns and 'turnover_rate' in df.columns:
                df['volume_turnover_ratio'] = df['volume_ratio'] / (df['turnover_rate'] + 1e-8)
            ...
            # 技术指标组合
            if all(col in df.columns for col in ['rsi_14', 'macd_signal', 'bollinger_position']):
                df['technical_score'] = (
                    (df['rsi_14'] - 50) / 50 * 0.3 +
                    df['macd_signal'] * 0.4 +
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, validation_split: float = 0.2) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Training completed. Val MSE: {val_mse:.4f}, Val R2: {val_r2:.4f}, Val IC: {val_ic:.4f}")
