...
本章详细说明量化导航仪系统的API接口设计，包括FastAPI后端接口、自动类型生成、契约管理等各个模块的接口规范。

> **相关文档**:
> - [开发文档第4章-技术架构.md](./开发文档第4章-技术架构.md) - 整体技术架构
> - [开发文档第8章-前端开发.md](./开发文档第8章-前端开发.md) - 前端开发与类型生成
> - [API_CHANGELOG.md](./API_CHANGELOG.md) - API变更日志

### 7.1.1 v13.8 API设计特点
- ✅ **FastAPI框架**: 现代化异步Web框架，自动API文档生成
- ✅ **自动类型生成**: 从OpenAPI自动生成TypeScript类型
- ✅ **契约管理**: 前后端契约自动同步
- ✅ **抽象接口**: 数据源和LLM提供商抽象接口
- ✅ **契约测试**: 自动化验证接口实现符合契约

## 7.2 FastAPI接口设计 (v13.8 极简主义最终版)

### 7.2.1 FastAPI应用结构

**应用入口**: `src/main.py`
- FastAPI应用实例创建
- 中间件配置 (CORS, TrustedHost)
- 路由注册
- CLI命令集成 (Typer)
...
/api/v1/reports/   # 报告管理API
/api/v1/workflow/  # 工作流API
/health            # 健康检查
/docs              # 自动生成的API文档
/redoc             # ReDoc API文档
```
...
**数据源抽象接口** (`src/core/interfaces/data_source_interface.py`):
```python
...
**LLM提供商抽象接口** (`src/core/interfaces/llm_provider_interface.py`):
```python
class LlmProviderInterface(ABC):
    ...
    @abstractmethod
    async def analyze_sentiment(self, text: str, model: str, **kwargs) -> Dict[str, Any]
    # ... 其他方法
```
...
- 自动启动FastAPI服务器
- 获取OpenAPI规范
- 使用openapi-typescript-codegen生成TypeScript类型
- 输出到frontend/src/types/api.ts
...
**契约测试生成器** (`tests/contracts/test_suite_generator.py`):
```python
def run_data_source_contract_tests(data_source: DataSourceInterface) -> bool
def run_llm_provider_contract_tests(llm_provider: LlmProviderInterface) -> bool
```
...
**架构依赖测试** (`tests/architecture/test_dependencies.py`):
- 禁止直接导入具体实现
- 确保使用抽象接口
- 检测循环依赖
- 验证契约测试存在
