...
from prediction_engine_main import PredictionEngineMain
...
class LearningLoopCoordinator:
    ...
    def run_historical_loop(self, start_date: str, end_date: str) -> Dict[str, Any]:
        """运行历史学习循环"""
        try:
            self.logger.info(f"Starting historical learning loop from {start_date} to {end_date}")
            ...
            while current_date < end_dt:
                ...
                self.logger.info(f"Starting iteration {iteration_count}: {current_date}")
                
                # 计算训练和验证日期范围
                train_end_date = current_date + timedelta(days=self.retrain_frequency_days)
                ...
                if iteration_result:
                    ...
                    if iteration_result.overall_ic > best_performance:
                        ...
                        self.logger.info(f"New best performance: IC={best_performance:.4f}")
            ...
            self.logger.info(f"Historical learning loop completed. Total iterations: {iteration_count}")
            return final_report
    ...
    def _execute_learning_iteration(self, 
                                  iteration_id: str,
                                  train_start: str,
                                  train_end: str,
                                  val_start: str,
                                  val_end: str) -> Optional[LearningIteration]:
        """执行单次学习迭代"""
        try:
            # 1. 加载训练数据
            ...
            if len(train_data) < self.min_training_samples:
                self.logger.warning(f"Insufficient training data: {len(train_data)} < {self.min_training_samples}")
                return None
            ...
            y_sentiment_train = np.where(y_train > 0.02, 1, np.where(y_train < -0.02, -1, 0))
            ...
            if val_predictions:
                ...
                self.logger.info(f"Iteration {iteration_id} completed: IC={ic:.4f}, Sharpe={sharpe:.4f}")
                return iteration
    ...
    def _load_training_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        return pd.read_sql_query(query, self.db_connection, params=[start_date, end_date])
    ...
    def _prepare_training_data(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, List[str]]:
        """准备训练数据"""
        # 特征列
        feature_columns = [
            'z_score_price', 'z_score_relative', 'volume_percentile',
    ...
    def _create_layer3_training_data(self, 
                                   data: pd.DataFrame,
    ...
    def _save_iteration_models(self, iteration_id: str, layer1: Layer1LightGBM, layer2: Layer2FinBERT, layer3: Layer3Llama3) -> None:
        """保存迭代模型"""
        try:
    ...
    def _generate_final_report(self, best_model_version: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for feature, importance_history in self.feature_evolution.items():
                if len(importance_history) > 1:
                    feature_evolution_summary[feature] = {
                        ...
                        'trend': 'increasing' if importance_history[-1] > importance_history[0] else 'decreasing',
                        'volatility': np.std(importance_history)
                    }
            
            # 性能趋势分析
            performance_trend = {
                'ic_trend': 'improving' if ic_values[-1] > ic_values[0] else 'declining',
            ...
            final_report = {
                ...
                'recommendations': self._generate_recommendations(performance_trend, feature_evolution_summary),
    ...
    def _generate_recommendations(self, performance_trend: Dict, feature_evolution: Dict) -> List[str]:
        """生成改进建议"""
        recommendations = []
        ...
        # 特征建议
        important_features = [f for f, info in feature_evolution.items() 
                            if info['final_importance'] > 0.1]
        if len(important_features) < 5:
            recommendations.append("重要特征数量较少，建议扩展特征工程")
        
        # 稳定性建议
        volatile_features = [f for f, info in feature_evolution.items() 
                           if info['volatility'] > 0.1]
...
def main():
    ...
    parser.add_argument('--config', default='learning_loop_config.json', help='配置文件路径')
    ...
    try:
        ...
        print(f"Learning loop completed: {json.dumps(report, indent=2, ensure_ascii=False)}")
