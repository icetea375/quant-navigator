...
import { PostProcessingValidator } from './PostProcessingValidator';
import { 
  UNIFIED_LLM_CONFIG, 
  LLMProvider, 
...
export class LLMServiceManager {
  ...
  private postProcessor: PostProcessingValidator;

  private constructor() {
    this.db = DatabaseConnection.getInstance();
    this.wsService = WebSocketService.getInstance();
    this.postProcessor = new PostProcessingValidator(this);
  }
  ...
  /**
   * 健壮的LLM调用方法 - 7步走的响应处理工作流
   */
  public async callLLMWithValidation(
    prompt: string,
    taskType: 'prediction_generation' | 'mda_verification' | 'counterfactual_validation' | 'event_chain_building' | 'content_extraction' | 'data_analysis' | 'chinese_processing' | 'fast_processing' | 'general' | 'etf_analysis' = 'general',
    options: SharedLLMOptions = {}
  ): Promise<SharedLLMResponse> {
    console.log(`🚀 开始健壮LLM调用: ${taskType}`);
    ...
    try {
      ...
      console.log(`📡 调用${selectedProvider.name}服务...`);
      const rawResponse = await this.callLLM(serviceName, prompt, {
        maxTokens: mergedOptions.maxTokens,
        temperature: mergedOptions.temperature,
        systemPrompt: mergedOptions.systemPrompt,
        timeout: mergedOptions.timeout
      ...
      console.log(`✅ 原始响应接收成功，长度: ${rawResponse.content.length}`);

      // 2. 获取Schema和阈值
      const schema = this.postProcessor.getSchemaForTaskType(taskType);
      const confidenceThreshold = this.postProcessor.getConfidenceThresholdForTaskType(taskType);

      // 3-7. 执行完整的验证流程
      console.log(`🔍 开始后处理验证...`);
      const validationResult = await this.postProcessor.validateResponse(
        rawResponse.content,
        schema,
        taskType,
        confidenceThreshold
      ...
      if (!validationResult.isValid) {
        throw new Error(`验证失败: ${validationResult.errors.map(e => e.message).join(', ')}`);
      }

      console.log(`✅ 验证通过，置信度: ${this.postProcessor.extractConfidenceScore(validationResult.repairedData)}`);

      return {
        content: JSON.stringify(validationResult.repairedData),
      ...
      console.error(`❌ 健壮LLM调用失败: ${taskType}`, error);
      
      // 根据错误类型提供更详细的错误信息
      if (error instanceof Error) {
        if (error.name === 'LlmResponseFormatError') {
          console.error('  错误类型: JSON格式错误');
        } else if (error.name === 'LlmResponseValidationError') {
          console.error('  错误类型: Schema验证错误');
        } else if (error.name === 'LlmLowConfidenceError') {
          console.error('  错误类型: 置信度过低');
        }
      }
