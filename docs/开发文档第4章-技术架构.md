...
## 4.1 系统架构概述 (v13.8 极简主义最终版)

> **相关文档**:
> - [架构设计-模块化单体.md](../架构设计-模块化单体.md) - 详细的模块化单体架构设计
> - [开发文档第6章-项目目录结构说明.md](./开发文档第6章-项目目录结构说明.md) - 项目目录结构
> - [开发文档第7章-API接口设计.md](./开发文档第7章-API接口设计.md) - API接口设计

### 4.1.0 极简主义核心特点 (v13.8 最终版)

**Python全栈极简主义架构**:
- ✅ **Python全栈**: 统一使用Python技术栈，消除多语言复杂性
- ✅ **FastAPI后端**: 现代化异步Web框架，自动API文档生成
- ✅ **抽象接口设计**: 数据源和LLM提供商抽象接口，防止锁定依赖
- ✅ **极简工具链**: Ruff + Pytest双工具质量保证体系
- ✅ **判例法配置**: 每个配置决策都有明确的法理依据
- ✅ **契约测试**: 自动化验证接口实现符合契约
- ✅ **架构守护**: 自动化检测架构违规，防止腐化
- ✅ **YAGNI平衡**: 在功能简化与架构抽象间找到平衡
...
graph TD
    ...
    subgraph "抽象接口层 (Interfaces)"
        DataSourceInterface[数据源接口]
        LLMProviderInterface[LLM提供商接口]
    end

    subgraph "具体实现层 (Implementations)"
        TushareFetcher[Tushare数据源]
        QwenProvider[Qwen提供商]
    end
    ...
    LLMService --> LLMProviderInterface
    DataService --> DataSourceInterface
    LLMProviderInterface --> QwenProvider
    DataSourceInterface --> TushareFetcher
    LLMService --> Config
    DataService --> Database
    WorkflowService --> Logging
    Ruff --> API
    Pytest --> ContractTests
    Pytest --> ArchitectureTests
    end

### 4.1.2 v13.8 技术栈说明

#### 后端技术栈
- **Web框架**: FastAPI (异步、自动API文档生成)
- **数据库**: PostgreSQL + SQLAlchemy ORM
- **缓存**: Redis (可选)
- **数据源**: Tushare API (通过抽象接口)
- **LLM提供商**: Qwen API (通过抽象接口)
...
#### 质量保证技术栈
- **静态分析**: Ruff (替代flake8+black+isort+mypy)
...
1. **【必选】原始文本浏览器 (Raw Text Explorer)** ✅ **已实现**
...
**技术实现**:
- **统一入口**: `config/settings.py` - 系统唯一的配置定义
- **类型安全**: `pydantic.BaseSettings` 提供自动验证
- **环境隔离**: 通过环境变量实现开发/测试/生产环境分离
