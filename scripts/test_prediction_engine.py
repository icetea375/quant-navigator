...
class PredictionEngineTester:
    ...
    async def create_mock_quant_signals(self):
        ...
        # 获取交易日列表
        self.cursor.execute("SELECT DISTINCT trade_date FROM daily_prices ORDER BY trade_date LIMIT 10")
    ...
    async def test_feature_generation(self):
        ...
        try:
            ...
            self.results['feature_generation']['details'].append(f"特征生成测试失败: {e}")
    ...
    async def generate_technical_features(self, data: List) -> Dict:
        ...
        features['volume_ratio'] = volumes[0] / np.mean(volumes[1:5]) if len(volumes) >= 5 else 1.0
    ...
    async def test_text_features(self):
        ...
        for feature_name, feature_value in text_features.items():
            if not isinstance(feature_value, (int, float)) or (isinstance(feature_value, float) and np.isnan(feature_value)):
    ...
    async def generate_text_features(self, news_data: List) -> Dict:
        ...
        for keyword in keywords:
            count = sum(1 for row in news_data 
    ...
    async def test_lightgbm_training(self):
        ...
        logger.info(f"LightGBM训练完成 - MSE: {training_metrics['mse']:.4f}, "
    ...
    async def test_finbert_analysis(self):
        ...
        for i, (title, content) in enumerate(news_texts):
            ...
            logger.info(f"新闻 {i+1} 情感分析 - 得分: {sentiment_score:.4f}, 置信度: {confidence:.4f}")
    ...
    async def test_llama3_decision(self):
        ...
        logger.info(f"Llama 3决策 - 预测: {final_prediction:.4f}, 置信度: {confidence:.4f}")
    ...
    async def test_prediction_generation(self):
        ...
        try:
            ...
            self.results['prediction_generation']['details'].append(f"预测生成测试失败: {e}")
    ...
    async def validate_predictions(self, predictions: List[Dict]):
        ...
        for prediction in predictions:
            ...
            if abs(prediction['predicted_return']) > 0.2:
                logger.warning(f"股票 {prediction['stock_code']} 预测收益率异常: {prediction['predicted_return']:.4f}")
            
            # 验证置信度范围
            if not 0 <= prediction['confidence'] <= 1:
                logger.warning(f"股票 {prediction['stock_code']} 置信度异常: {prediction['confidence']:.4f}")
    ...
    async def test_confidence_assessment(self):
        ...
        try:
            ...
            self.results['confidence_assessment']['details'].append("置信度评估测试通过")
            logger.info("置信度评估测试通过")
            
        except Exception as e:
            self.results['confidence_assessment']['status'] = 'failed'
            self.results['confidence_assessment']['details'].append(f"置信度评估测试失败: {e}")
    ...
    async def test_confidence_distribution(self):
        ...
        logger.info(f"置信度分布 - 均值: {mean_confidence:.4f}, 标准差: {std_confidence:.4f}")
    ...
    async def generate_test_report(self):
        ...
        # 保存报告
        report_file = f"prediction_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        
        logger.info(f"测试报告已保存: {report_file}")
        logger.info(f"PredictionEngine测试完成 - 通过率: {report['summary']['pass_rate'] * 100:.1f}%, 耗时: {total_duration:.2f}秒")
