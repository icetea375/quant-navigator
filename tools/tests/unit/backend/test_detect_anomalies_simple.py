...
class MockMainWorkflow:
    ...
    async def _detect_anomalies(self, trade_date: str) -> list:
        """
...
class TestDetectAnomalies:
    ...
    @pytest.mark.asyncio
    async def test_green_phase_detect_anomalies_success(self, mock_workflow):
        """测试成功检测异常股票"""
        # 模拟量化引擎返回异常股票
    ...
    async def test_detect_anomalies_exception_logging(self, mock_workflow):
        """测试异常时的日志记录"""
        # 模拟一个会触发critical日志的场景
        # 让第一个任务成功,第二个任务失败,这样会触发我们的异常处理逻辑
        mock_workflow.quant_engine.detect_anomalies_async.return_value = ["000001"]
        ...
        # 验证info日志被调用
        mock_workflow.logger.info.assert_called_with(f"开始检测{trade_date}的异常股票")
