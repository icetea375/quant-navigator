...
class TestDataPipelineStorageIntegration:
    ...
    @pytest.mark.asyncio
    async def test_green_phase_save_financial_factors_success(
        self, data_pipeline_service, sample_financial_factors
    ):
        """测试成功保存财务因子"""
        result = await data_pipeline_service.save_financial_factors(
            sample_financial_factors
        )

        assert result is True
        assert isinstance(result, bool)  # TODO: 替换为具体的True/False断言

    @pytest.mark.asyncio
    async def test_save_financial_factors_missing_required_field(
    ...
    @pytest.mark.asyncio
    async def test_save_financial_factors_database_error(
        self, data_pipeline_service, sample_financial_factors
    ):
        """测试保存财务因子时数据库错误"""
        with patch.object(
            ...
            with pytest.raises(Exception) as exc_info:
                await data_pipeline_service.save_financial_factors(
                    sample_financial_factors
                )
    ...
    @pytest.mark.asyncio
    async def test_save_super_financial_factors_success(
        self, data_pipeline_service, sample_super_financial_factors
    ):
        """测试成功保存超级财务因子"""
        result = await data_pipeline_service.save_super_financial_factors(
            sample_super_financial_factors
        )
    ...
    @pytest.mark.asyncio
    async def test_save_super_financial_factors_missing_required_field(
        self, data_pipeline_service
    ):
        """测试保存超级财务因子时缺少必要字段"""
        incomplete_data = {
    ...
    @pytest.mark.asyncio
    async def test_save_super_financial_factors_database_error(
        self, data_pipeline_service, sample_super_financial_factors
    ):
        """测试保存超级财务因子时数据库错误"""
        with patch.object(
            data_pipeline_service,
            "_persist_super_financial_factors",
            side_effect=Exception("数据库连接失败"),
        ):
            with pytest.raises(Exception) as exc_info:
                await data_pipeline_service.save_super_financial_factors(
                    sample_super_financial_factors
                )
    ...
    @pytest.mark.asyncio
    async def test_persist_super_financial_factors_validation(
        self, data_pipeline_service
    ):
        """测试超级财务因子数据验证逻辑"""
        # 测试所有必要字段都存在
    ...
    @pytest.mark.asyncio
    async def test_save_financial_factors_with_negative_values(
        self, data_pipeline_service
    ):
        """测试保存包含负值的财务因子"""
        data_with_negatives = {
    ...
    @pytest.mark.asyncio
    async def test_save_super_financial_factors_with_edge_scores(
        self, data_pipeline_service
    ):
        """测试保存边界评分的超级财务因子"""
    ...
    @pytest.mark.asyncio
    async def test_save_financial_factors_performance(
        self, data_pipeline_service, sample_financial_factors
    ):
        """测试财务因子保存性能"""
        import time
    ...
    @pytest.mark.asyncio
    async def test_save_super_financial_factors_performance(
        self, data_pipeline_service, sample_super_financial_factors
    ):
        """测试超级财务因子保存性能"""
        import time
