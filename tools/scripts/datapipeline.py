...
class CompleteDataLoader:
        ...
        try:
            ...
            if not industry_data.empty:
                for _, row in industry_data.iterrows():
                    ...
                    """,
                        (row["index_code"], row["industry_name"], 1, None),
                    )
                self.conn.commit()
            ...
            for index_code in index_list:
                try:
                    ...
                    if not weight_data.empty:
                        ...
                        self.conn.commit()
                        logger.info(
                            f"指数 {index_code} 权重数据加载完成: {len(weight_data)} 条"
                        )
    ...
    async def load_daily_market_data(self):
        ...
        try:
            ...
            # 获取股票列表
            stock_list = self.ts.stock_basic(
                ...
                for ts_code in batch_stocks:
                    try:
                        ...
                        if not daily_data.empty:
                            ...
                            self.conn.commit()
                            logger.info(
                                f"股票 {ts_code} 日线数据加载完成: {len(daily_data)} 条"
                            )
            ...
            for index_code in index_list:
                try:
                    ...
                    if not index_daily_data.empty:
                        ...
                        self.conn.commit()
                        logger.info(
                            f"指数 {index_code} 日线数据加载完成: {len(index_daily_data)} 条"
                        )
            ...
            for stock_code in stock_codes:
                try:
                    ...
                    if not money_flow_data.empty:
                        ...
                        sell_elg_amount = EXCLUDED.sell_elg_amount
                            """,
                                (
                                    ...
                                    row.get("net_mf_amount", 0)
                                    / row.get("trade_money", 1)
                                    if row.get("trade_money", 0) > 0
                                    else 0,
                        ...
                        self.conn.commit()
                        logger.info(
                            f"股票 {stock_code} 资金流向数据加载完成: {len(money_flow_data)} 条"
                        )
        ...
        try:
            ...
            if not jg_gold_data.empty:
                for _, row in jg_gold_data.iterrows():
                    ...
                    """,
                        (
                            datetime.strptime(str(row["month"]), "%Y%m")
                            .date()
                            .replace(day=1),
                            row["ts_code"],
                            row["name"],
