...
class DocstringQualityChecker:
    ...
    def check_file(self, file_path: Path) -> List[Dict]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                if docstring:
                    quality_issues = self._check_docstring_quality(docstring, node.name, file_path)
    ...
    def _check_docstring_quality(self, docstring: str, function_name: str, file_path: Path) -> List[Dict]:
        """检查单个docstring的质量"""
        issues = []
        ...
        # 检查是否包含"为什么"的解释
        has_why_explanation = any(keyword in docstring for keyword in self.required_keywords)
        if not has_why_explanation:
            issues.append({
                "type": "missing_why_explanation",
                "file": str(file_path),
                "function": function_name,
                "message": f"docstring缺少'为什么'的解释，需要说明函数存在的业务目的"
            })
        ...
        if "Args:" in docstring and "Returns:" in docstring:
            # 检查是否有业务背景说明
            business_context = any(keyword in docstring for keyword in ["业务", "需求", "问题", "解决", "目的"])
...
def main():
    ...
    parser.add_argument("--path", default="packages/backend-python/src", help="检查路径")
