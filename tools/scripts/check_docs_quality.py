...
class DocsQualityChecker:
    ...
    def check_adr_completeness(self) -> None:
        ...
        if not adr_file.exists():
            self.issues.append(
                {"type": "missing_adr_file", "file": "docs/ADR.md", "severity": "error"}
            )
            return
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        """判断是否应该跳过某个文件"""
        skip_patterns = ["__pycache__", ".git", "test_", "_test.py", "conftest.py"]
    ...
    def _has_docstring(self, node) -> bool:
        ...
        if isinstance(node, ast.ClassDef):
            return (
                ...
                and len(node.body[0].value.value.strip()) > 0
            )
        elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            return (
                node.body
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"- 有docstring的函数数: {self.stats['functions_with_docstrings']}"
        )
        ...
        if self.stats["total_functions"] > 0:
            docstring_coverage = (
                self.stats["functions_with_docstrings"] / self.stats["total_functions"]
            ) * 100
        ...
        if self.issues:
            ...
            if error_issues:
                ...
                for issue in error_issues:
                    report.append(
                        f"- **{issue['file']}**: {issue.get('issue', issue.get('type', '未知问题'))}"
                    )
                    if "line" in issue:
...
def main():
    ...
    # 根据问题数量设置退出码
    error_count = len(
        [issue for issue in checker.issues if issue["severity"] == "error"]
    )
    if error_count > 0:
        sys.exit(1)
    else:
