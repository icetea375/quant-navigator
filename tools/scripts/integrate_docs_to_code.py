...
class DocsToCodeIntegrator:
    ...
    def __init__(self, project_root: str):
        ...
        self.docs_dir = self.project_root / "docs_migration_staging" / "to_be_docstring_or_code_comment"
    ...
    def generate_docstring_content(self, analysis: Dict, target_file: Path) -> str:
        ...
        for section in sections[:3]:  # 只取前3个章节
            if section["title"] and len(section["content"]) > 0:
                ...
                # 取章节内容的前几行
                content_lines = [line.strip() for line in section["content"][:3] if line.strip()]
    ...
    def generate_integration_report(self, results: List[Dict]):
        ...
        # 保存报告
        report_file = self.project_root / "docs_migration_staging" / "文档整合到代码报告.md"
