...
class ArbitrationPreprocessor:
    ...
    def process_daily_arbitration_cases(self, trade_date: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "trade_date": trade_date, "error": str(e)}
    ...
    def _process_report_pairs(
        self, report_pairs: List[Dict[str, Any]], trade_date: str
    ) -> Dict[str, Any]:
        """
        处理报告对列表
        ...
        for pair in report_pairs:
            try:
                ...
                comparison_result = self.report_comparator.compare_reports(
                    pair["qwen_report"], pair["doubao_report"]
                )

                # 2. 计算仲裁优先级
                priority_score = self._calculate_arbitration_priority(
                    pair["stock_code"], comparison_result
                )
                ...
                self.logger.info(
                    f"成功处理股票{pair['stock_code']}，优先级: {priority_score:.3f}"
                )
                successful += 1

            except Exception as e:
                self.logger.error(
                    f"处理股票{pair['stock_code']}失败: {e}", exc_info=True
                )
                failed_cases.append({"stock_code": pair["stock_code"], "error": str(e)})
                failed += 1
                continue
    ...
    def _calculate_arbitration_priority(
        self, stock_code: str, comparison_result: ComparisonResult
    ) -> float:
        """
        计算仲裁优先级
        ...
        try:
            ...
            self.logger.debug(
                f"股票{stock_code}优先级计算: 分歧度={divergence_score:.3f}, "
    ...
    def _generate_processing_report(
        self, trade_date: str, results: Dict[str, Any]
    ) -> None:
        """
        生成处理报告
