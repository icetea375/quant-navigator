...
投资建议：HOLD - 市场情绪谨慎，建议等待明确信号""",  # 分析文本
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            # 创建仲裁案件
            test_case_id = (
                f"ARB_{test_case['stock_code']}_{date.today().strftime('%Y%m%d')}"
            )
            await conn.execute(
                arbitration_case_query,
                test_case_id,
        ...
        print(
            f"  POST /api/v1/admin/arbitration-cases/{case_id}/feedback - 提交仲裁判决"
        )
