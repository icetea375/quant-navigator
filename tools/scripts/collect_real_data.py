...
class RealDataCollector:
    ...
    def _store_real_data(self, arbitration_data: List[Dict], 
                        api_data: List[Dict], error_data: List[Dict]):
        """存储真实数据到数据库"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        for item in arbitration_data:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, ("UNKNOWN", datetime.now().date(), 
                  datetime.now(), datetime.now(), 0.0, "log_parsing"))
