...
class QwenFactAnalyzer:
    ...
    def analyze(self, stock_code: str, trade_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            report = self._generate_final_report(mda_scores, event_chain, stock_code, trade_date)
            
            self.logger.info(f"Qwen事实归因分析完成: {stock_code}")
            return report
            
        except Exception as e:
            self.logger.error(f"Qwen事实归因分析失败: {stock_code} - {e}", exc_info=True)
            raise
    
    def _load_internal_data_for_stock(self, stock_code: str, trade_date: str) -> Dict[str, Any]:
        """
        从数据库加载股票的内部结构化数据
        ...
        try:
            ...
            # 加载公告数据
            announcement_data = self.db_manager.get_announcement_data(stock_code, trade_date)
    ...
    def _verify_mda(self, internal_data: Dict[str, Any], stock_code: str, trade_date: str) -> Dict[str, Any]:
        """
        使用Qwen验证MD&A数据
        ...
        try:
            ...
            # 构建MD&A验证提示词
            prompt = self._build_mda_verification_prompt(mda_data, financial_data, stock_code, trade_date)
    ...
    def _build_event_chain(self, internal_data: Dict[str, Any], stock_code: str, trade_date: str) -> Dict[str, Any]:
        """
        使用Qwen构建事件链
        ...
        try:
            # 构建事件链构建提示词
            prompt = self._build_event_chain_prompt(internal_data, stock_code, trade_date)
    ...
    def _generate_final_report(self, mda_scores: Dict[str, Any], event_chain: Dict[str, Any], stock_code: str, trade_date: str) -> Dict[str, Any]:
        """
        生成最终的事实归因报告
        ...
        try:
            # 构建最终报告生成提示词
            prompt = self._build_final_report_prompt(mda_scores, event_chain, stock_code, trade_date)
    ...
    def _build_mda_verification_prompt(self, mda_data: Dict[str, Any], financial_data: Dict[str, Any], stock_code: str, trade_date: str) -> str:
        """
        构建MD&A验证提示词
        """
        prompt = f"""
...
"""
    ...
    def _build_event_chain_prompt(self, internal_data: Dict[str, Any], stock_code: str, trade_date: str) -> str:
        """
        构建事件链构建提示词
        """
        prompt = f"""
...
"""
    ...
    def _build_final_report_prompt(self, mda_scores: Dict[str, Any], event_chain: Dict[str, Any], stock_code: str, trade_date: str) -> str:
        """
        构建最终报告生成提示词
        """
        prompt = f"""
