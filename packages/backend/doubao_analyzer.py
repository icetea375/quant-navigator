...
class DoubaoSentimentAnalyzer:
    ...
    def analyze(self, stock_code: str, trade_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. 调用豆包进行舆情感知分析
            response = self._call_doubao_sentiment_analysis(prompt, stock_code, trade_date)
            ...
            self.logger.error(f"豆包舆情感知分析失败: {stock_code} - {e}", exc_info=True)
            raise
...
"""
    ...
    def _call_doubao_sentiment_analysis(self, prompt: str, stock_code: str, trade_date: str) -> str:
        """
        调用豆包进行舆情感知分析
        ...
        try:
            ...
            self.logger.error(f"豆包舆情感知分析调用失败: {stock_code} - {e}", exc_info=True)
            raise
    
    def _parse_doubao_response(self, response: str, stock_code: str, trade_date: str) -> Dict[str, Any]:
        """
        解析豆包响应并生成结构化报告
        ...
        try:
            ...
            report = {
                ...
                'risk_assessment': {
                    ...
                    'black_swan_probability': parsed_data.get('black_swan_probability', 0.1),
                    'volatility_forecast': parsed_data.get('volatility_forecast', 'medium')
                },
                'investment_implications': {
                    'position_recommendation': parsed_data.get('position_recommendation', 'HOLD'),
                    'risk_adjustment': parsed_data.get('risk_adjustment', 'neutral'),
                    'timing_considerations': parsed_data.get('timing_considerations', ''),
    ...
    def _create_default_report(self, stock_code: str, trade_date: str, response: str) -> Dict[str, Any]:
        """
        创建默认报告结构
        """
        return {
