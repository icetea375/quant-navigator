...
class MainWorkflow:
    ...
    def run_daily_flow(self, trade_date: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 5. 生成执行报告
            self._generate_execution_report(trade_date, anomaly_stocks, results, arbitration_results)
            ...
            return {
                ...
                "arbitration_cases_processed": arbitration_results.get("processed_count", 0),
    ...
    def _save_report_to_db(self, report: Dict[str, Any], source: str) -> None:
        ...
        try:
            ...
            if 'summary' not in report:
                report['summary'] = report.get('content', '')[:500]  # 使用内容前500字符作为摘要
    ...
    def _execute_arbitration_preprocessing(self, trade_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 调用仲裁预处理器处理当日案件
            result = self.arbitration_preprocessor.process_daily_arbitration_cases(trade_date)
            
            if result['status'] == 'success':
                self.logger.info(f"仲裁预处理完成: 处理了{result.get('processed_count', 0)}个案件")
            else:
    ...
    def _generate_execution_report(self, trade_date: str, anomaly_stocks: List[str], results: Dict[str, Any], arbitration_results: Dict[str, Any]) -> None:
        """
        生成执行报告
        ...
        if arbitration_results['status'] == 'success':
            self.logger.info(f"仲裁案件处理成功: {arbitration_results.get('processed_count', 0)}")
            self.logger.info(f"仲裁案件处理失败: {arbitration_results.get('failed_count', 0)}")
        else:
            self.logger.error(f"仲裁预处理失败: {arbitration_results.get('error', '未知错误')}")
