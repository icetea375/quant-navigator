...
{output}<|eot_id|><|end_of_text|>"""
    ...
    def load_pretrained_model(self, model_name: str = "meta-llama/Llama-3-8B-Instruct") -> None:
        """加载预训练模型"""
        try:
    ...
    def predict(self, 
        ...
        try:
            ...
            prompt = self.create_prediction_prompt(
                ...
                p1_prediction, p2_prediction, news_summary, market_context
            )
    ...
    def prepare_training_data(self, 
                            training_samples: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        """准备训练数据"""
        try:
            training_data = []
    ...
    def train(self, training_samples: List[Dict[str, Any]], validation_split: float = 0.2) -> Dict[str, Any]:
        """训练模型"""
        try:
            if self.model is None:
                self.load_pretrained_model()
            
            self.logger.info(f"Training Llama 3 model with {len(training_samples)} samples")
            ...
            self.logger.info(f"Training completed. Eval loss: {eval_results['eval_loss']:.4f}")
    ...
    def _create_dataset(self, data: List[Dict[str, str]]) -> torch.utils.data.Dataset:
        ...
        return TextDataset([item['text'] for item in data], self.tokenizer, self.max_tokens)
