...
class WorkflowConfig(BaseModel):
    ...
    observation_universe_max_size: int = Field(
        500, ge=1, description="观察宇宙最大规模"
    )
    ...
    @field_validator("promotion_check_time", "demotion_check_time")
    @classmethod
...
class SignalTranslationConfig(BaseModel):
    ...
    enabled: bool = Field(True, description="是否启用")
    translation_rules: List[Dict[str, Any]] = Field(
        default_factory=list, description="转换规则"
    )
...
class AttributionEngineConfig(BaseModel):
    ...
    anomaly_detection: AnomalyDetectionConfig = Field(
        default_factory=AnomalyDetectionConfig, description="异常检测配置"
    )
    signal_translation: SignalTranslationConfig = Field(
        default_factory=SignalTranslationConfig, description="信号转换配置"
    )
    llm_collaboration: LLMCollaborationConfig = Field(
        default_factory=LLMCollaborationConfig, description="LLM协作配置"
    )
    workflow: WorkflowExecutionConfig = Field(
        default_factory=WorkflowExecutionConfig, description="工作流配置"
    )
