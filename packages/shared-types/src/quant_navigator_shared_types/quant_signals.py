...
class QuantSignal(BaseModel):
    ...
    management_credibility_score: float = Field(
        ..., ge=0.0, le=1.0, description="管理层可信度分数"
    )
    disclosure_quality_score: float = Field(
        ..., ge=0.0, le=1.0, description="披露质量分数"
    )
    financial_transparency_score: float = Field(
        ..., ge=0.0, le=1.0, description="财务透明度分数"
    )
    ...
    overall_signal_strength: float = Field(
        ..., ge=-1.0, le=1.0, description="整体信号强度"
    )
    ...
    calculation_params: Dict[str, Any] = Field(
        default_factory=dict, description="计算参数"
    )
...
class SignalFilter(BaseModel):
    ...
    min_confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="最小置信度"
    )
    max_confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="最大置信度"
    )
    min_strength: Optional[float] = Field(
        None, ge=-1.0, le=1.0, description="最小信号强度"
    )
    max_strength: Optional[float] = Field(
        None, ge=-1.0, le=1.0, description="最大信号强度"
    )
...
class SignalBatch(BaseModel):
    ...
    def validate_total_count(cls, v, info):
        ...
        if (
            hasattr(info, "data")
            and "signals" in info.data
            and v != len(info.data["signals"])
        ):
