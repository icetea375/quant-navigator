...
class GeneratedReport(BaseModel):
    ...
    metrics: ReportMetrics = Field(
        default_factory=ReportMetrics, description="报告指标"
    )

    # 关联数据
    related_events: List[str] = Field(
        default_factory=list, description="相关事件ID列表"
    )
    related_signals: List[str] = Field(
        default_factory=list, description="相关信号ID列表"
    )
    ...
    file_format: Literal["pdf", "html", "json", "markdown"] = Field(
        "json", description="文件格式"
    )
