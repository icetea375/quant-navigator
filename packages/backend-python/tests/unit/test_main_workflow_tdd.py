...
class TestMainWorkflowTDD:
    ...
    def mock_workflow(self, mock_config):
        ...
        # 模拟所有依赖服务
        with patch("src.main_workflow.DataPipelineService") as mock_data_pipeline, \
    ...
    @pytest.mark.asyncio
    async def test_run_daily_flow_should_process_one_stock_successfully(self, mock_workflow):
        """
        ...
        # 模拟并行处理成功
        mock_workflow._process_anomaly_stocks = AsyncMock(return_value={
            "successful": 1,
            "failed": 0,
            "failed_stocks": [],
        ...
        mock_workflow._process_anomaly_stocks.assert_called_once_with(["000001"], "2025-01-17")
        mock_workflow._generate_execution_report.assert_called_once()

    # ==================== 第二个测试:异常检测失败 ====================

    @pytest.mark.asyncio
    async def test_run_daily_flow_should_handle_detect_anomalies_failure(self, mock_workflow):
        """
        红灯测试:异常检测失败的情况
    ...
    async def test_load_stock_data_should_respect_concurrency_limit(self, mock_workflow):
        ...
        """
        # 模拟数据源
        mock_workflow.data_pipeline.get_financial_data_async = AsyncMock(return_value={"revenue": 1000})
    ...
    async def test_process_single_stock_with_retry_should_succeed(self, mock_workflow):
        ...
        """
        # 模拟数据加载成功
        mock_workflow._load_stock_data = AsyncMock(return_value={
            "stock_code": "000001",
