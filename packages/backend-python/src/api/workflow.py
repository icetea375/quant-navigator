...
async def run_daily_flow(background_tasks: BackgroundTasks, target_date: str = None):
    """运行日常分析工作流"""
    try:
        ...
        # 在后台运行工作流
        background_tasks.add_task(service.run_daily_flow, target_date)
...
async def run_historical_backfill(
    background_tasks: BackgroundTasks, start_date: str, end_date: str
):
    """运行历史数据回填工作流"""
    try:
        service = WorkflowService()
        ...
        # 在后台运行历史回填
        background_tasks.add_task(service.run_historical_backfill, start_date, end_date)
...
async def get_workflow_status(workflow_id: str):
    ...
    try:
        ...
        return {"success": True, "message": "获取工作流状态成功", "data": status}
