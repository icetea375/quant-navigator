"""
豆包 LLM提供商实现 - 实现LlmProviderInterface接口
...
class DoubaoProvider(LlmProviderInterface):
    ...
    def __init__(
        ...
        Args:
            api_key: 豆包 API密钥,如果为None则从配置中读取
            base_url: API基础URL,如果为None则使用默认值
            client: HTTP客户端,如果为None则创建默认客户端
        """
        self.logger = logging.getLogger(__name__)
        self.api_key = api_key or getattr(settings, "DOUBAO_API_KEY", "")
        ...
        **kwargs,
        ...
        try:
            ...
            if response.status_code == 401:
                raise LlmProviderAuthenticationError("豆包 API认证失败")
            elif response.status_code == 429:
                raise LlmProviderRateLimitError("豆包 API请求频率超限")
            elif response.status_code != 200:
                raise LlmProviderError(f"豆包 API请求失败: {response.status_code}")
            ...
            if "choices" in result and len(result["choices"]) > 0:
                ...
                raise LlmProviderError("豆包 API响应格式错误")

        except httpx.TimeoutException as e:
            self._error_count += 1
            raise LlmProviderTimeoutError("豆包 API请求超时") from e
            ...
            if isinstance(
                ...
                raise LlmProviderError(f"豆包 API调用失败: {e!s}") from e
    ...
    async def generate_embeddings(
        self, texts: list[str], model: str = "doubao-embedding", **kwargs
    ) -> list[list[float]]:
        """
        生成文本嵌入向量
    ...
    async def analyze_sentiment(
        self, text: str, model: str = "doubao-seed-1-6", **kwargs
    ) -> dict[str, Any]:
        """
        情感分析
    ...
    async def batch_process(
        self, requests: list[dict[str, Any]], model: str = "doubao-seed-1-6", **kwargs
    ) -> list[dict[str, Any]]:
        """
        批量处理请求
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            # 测试API连接
            await self.generate_text("测试", "doubao-seed-1-6", max_tokens=10)
            ...
            return {
                ...
                "rate_limit_remaining": 999,  # 豆包的限流信息需要额外获取
