...
from quant_navigator_shared_types.events import AnomalyEvent
from sqlalchemy import JSON, CheckConstraint, Column, Float, Integer, String, Text
...
class AnomalyEventEntity(BaseEntity):
    ...
    def _validate_input_data(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if "context_json" in validation_data:
                validation_data["context"] = validation_data.pop("context_json")
            if "metadata_json" in validation_data:
                validation_data["metadata"] = validation_data.pop("metadata_json")

            # 创建Pydantic模型进行验证
            AnomalyEvent(**validation_data)
        except ValidationError as e:
            raise ValueError(f"Invalid anomaly event data: {e}") from e
