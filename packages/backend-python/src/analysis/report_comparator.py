...
class ReportComparator:
    ...
    def calculate_divergence_score(
        self, qwen_report: Dict[str, Any], doubao_report: Dict[str, Any]
    ) -> float:
        """
        计算分歧度分数 - 多维度加权算法
        ...
        try:
            ...
            # 1. 计算情感差异
            sentiment_diff = self._calculate_sentiment_difference(
    ...
    def _calculate_sentiment_difference(
        self, qwen_report: Dict[str, Any], doubao_report: Dict[str, Any]
    ) -> float:
        """计算情感差异分数"""
        try:
    ...
    def _calculate_entity_difference(
        self, qwen_report: Dict[str, Any], doubao_report: Dict[str, Any]
    ) -> float:
        """计算核心实体差异"""
        try:
    ...
    def _extract_sentiment_score(self, report: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if text_content:
                ...
                positive_count = sum(
                    1 for word in positive_words if word in text_content
                )
                negative_count = sum(
                    1 for word in negative_words if word in text_content
                )
