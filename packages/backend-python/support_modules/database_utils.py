...
class DatabaseManager:
    ...
    def execute_delete(
        self, table_name: str, where_clause: str, where_params: Dict[str, Any] = None
    ) -> int:
        """
        执行删除操作
    ...
    def _save_prediction_results(self, results: Dict[str, Any]):
        ...
        try:
            ...
            report_data = {
                ...
                "summary": "基于多场景分析，预测未来5日收益率",
                "content": json.dumps(prediction, ensure_ascii=False),
                "confidence_score": prediction.get("probabilities", {}).get(
                    "optimistic", 0
                ),
    ...
    def get_arbitration_case_data(self, case_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            panels = {
                ...
                "quant_signal_dashboard": self._get_quant_signals(
                    stock_code, report_date
                ),
                "flow_and_chips_viewer": self._get_flow_and_chips_data(
                    stock_code, report_date
                ),
                "precedent_viewer": self._get_historical_arbitrations(
                    stock_code, report_date
                ),
            }
    ...
    def _get_ai_debate_data(self, stock_code: str, report_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self.execute_query(
                query, {"stock_code": stock_code, "report_date": report_date}
            )
    ...
    def _get_raw_text_data(
        ...
        try:
            ...
            start_date = (
                datetime.strptime(report_date, "%Y-%m-%d") - timedelta(days=30)
            ).strftime("%Y-%m-%d")
            end_date = (
    ...
    def _get_flow_and_chips_data(
        ...
        try:
            ...
            flow_data = self.execute_query(
                flow_query, {"stock_code": stock_code, "report_date": report_date}
            )
            ...
            top_list_data = self.execute_query(
                top_list_query, {"stock_code": stock_code, "report_date": report_date}
            )
            ...
            chip_data = self.execute_query(
                chip_query, {"stock_code": stock_code, "report_date": report_date}
            )
    ...
    def _get_historical_arbitrations(
        self, stock_code: str, report_date: str
    ) -> List[Dict[str, Any]]:
        """获取历史仲裁记录"""
        try:
            ...
            start_date = (
                datetime.strptime(report_date, "%Y-%m-%d") - timedelta(days=365)
            ).strftime("%Y-%m-%d")

            return self.execute_query(
                query, {"stock_code": stock_code, "start_date": start_date}
            )
