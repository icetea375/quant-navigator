...
class CausalValidator:
    ...
    def _load_prompt_templates(self) -> Dict[str, Any]:
        """加载反事实验证的Prompt模板"""
        try:
            template_path = Path("config/prompt_templates/counterfactual_validation_prompts.json")
    ...
    def validate(self, stock_code: str, trade_date: str, prediction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_empty_validation(stock_code, trade_date, prediction_result)
    
    def _perform_counterfactual_analysis(self, stock_code: str, trade_date: str, 
        ...
        try:
            ...
            prompt = self._build_counterfactual_prompt(
                template, stock_code, prediction_result, decision_package, key_positive_factor
            )
            ...
            return {
                ...
                "counterfactual_scenarios": self._generate_counterfactual_scenarios(risk_factors),
    ...
    def _build_counterfactual_prompt(self, template: Dict[str, Any], stock_code: str,
                                   prediction_result: Dict[str, Any], decision_package: Dict[str, Any],
                                   key_positive_factor: str) -> str:
        """构建反事实验证Prompt"""
        role = template["role"]
        ...
        # 准备输入数据
        prediction_json = json.dumps(prediction_result.get("prediction", {}), ensure_ascii=False, indent=2)
        full_decision_package_json = json.dumps(decision_package, ensure_ascii=False, indent=2)
        
        # 替换变量
        input_template = input_template.replace("{{prediction_json}}", prediction_json)
        input_template = input_template.replace("{{full_decision_package_json}}", full_decision_package_json)
...
请严格按照JSON格式输出，不要包含任何其他内容。"""
    ...
    def _get_default_risk_factors(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "risk_factor": "市场情绪：整体市场风险偏好可能下降",
                "likelihood": "低"
            }
        ]
    
    def _generate_counterfactual_scenarios(self, risk_factors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """生成反事实场景"""
        scenarios = []
    ...
    def _identify_risk_factors(self, stock_code: str, trade_date: str, 
                              prediction_result: Dict[str, Any]) -> List[Dict[str, Any]]:
        """识别风险因素"""
        try:
            # 基于预测结果识别风险
            prediction = prediction_result.get("prediction", {})
    ...
    def _perform_causal_analysis(self, stock_code: str, trade_date: str, 
                                prediction_result: Dict[str, Any]) -> Dict[str, Any]:
        """执行因果推断分析"""
        try:
            ...
            # 使用DoWhy进行因果推断
            return self._perform_dowhy_analysis(stock_code, trade_date, prediction_result)
            
        except Exception as e:
            self.logger.error(f"因果推断分析失败: {e}")
            return {"error": "Causal analysis failed"}
    
    def _perform_simple_causal_analysis(self, prediction_result: Dict[str, Any]) -> Dict[str, Any]:
        """执行简化的因果推断分析"""
        try:
            prediction = prediction_result.get("prediction", {})
    ...
    def _perform_dowhy_analysis(self, stock_code: str, trade_date: str, 
                               prediction_result: Dict[str, Any]) -> Dict[str, Any]:
        """使用DoWhy进行因果推断分析"""
        try:
            # 这里应该实现完整的DoWhy分析
    ...
    def _adjust_prediction_for_risks(self, prediction_result: Dict[str, Any], 
                                   risk_factors: List[Dict[str, Any]], 
                                   counterfactual_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """基于风险因素调整预测"""
        try:
            ...
            if risk_adjustment > 0:
                # 增加悲观概率，减少乐观概率
                probabilities["pessimistic"] = min(probabilities.get("pessimistic", 0) + risk_adjustment, 1.0)
                probabilities["optimistic"] = max(probabilities.get("optimistic", 0) - risk_adjustment, 0.0)
                probabilities["neutral"] = 1.0 - probabilities["pessimistic"] - probabilities["optimistic"]
            
            adjusted_prediction["probabilities"] = probabilities
            
            # 调整收益率
            scenarios = ["scenario_optimistic", "scenario_neutral", "scenario_pessimistic"]
    ...
    def _calculate_risk_adjustment(self, risk_factors: List[Dict[str, Any]]) -> float:
        ...
        try:
            ...
            # 基于风险因素数量和严重程度计算调整系数
            high_risk_count = sum(1 for risk in risk_factors if risk.get('likelihood') == '高')
            medium_risk_count = sum(1 for risk in risk_factors if risk.get('likelihood') == '中')
            low_risk_count = sum(1 for risk in risk_factors if risk.get('likelihood') == '低')
            
            # 计算调整系数
            adjustment = (high_risk_count * 0.15 + medium_risk_count * 0.08 + low_risk_count * 0.03)
    ...
    def _calculate_validation_score(self, counterfactual_analysis: Dict[str, Any], 
                                  risk_factors: List[Dict[str, Any]], 
        ...
        try:
            ...
            if counterfactual_analysis.get("risk_factors"):
                counterfactual_score = min(len(counterfactual_analysis["risk_factors"]) / 3, 1.0) * 0.2
    ...
    def _create_empty_validation(self, stock_code: str, trade_date: str, 
                               prediction_result: Dict[str, Any]) -> Dict[str, Any]:
        """创建空验证结果"""
        return {
            "stock_code": stock_code,
    ...
    def get_validation_summary(self, validation_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            counterfactual_analysis = validation_result.get("counterfactual_analysis", {})
            ...
            return {
                ...
                "has_counterfactual_analysis": bool(counterfactual_analysis.get("risk_factors"))
            }
