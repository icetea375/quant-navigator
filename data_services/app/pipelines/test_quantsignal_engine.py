...
class QuantSignalEngineTester:
    ...
    async def check_data_completeness(self):
        ...
        completeness = total_records / theoretical_records if theoretical_records > 0 else 0
    ...
    async def test_macro_risk_signals(self):
        ...
        if historical_volatility > 0:
            macro_risk_z = (volatility - historical_volatility) / (historical_volatility * 0.1)
        else:
            macro_risk_z = 0.0
    ...
    async def calculate_quant_fingerprint(self, stock_code: str) -> float:
        ...
        volume_trend = np.mean(volumes[-10:]) / np.mean(volumes[-30:]) if len(volumes) >= 30 else 1
        amount_trend = np.mean(amounts[-10:]) / np.mean(amounts[-30:]) if len(amounts) >= 30 else 1
        
        # 综合量化指纹
        fingerprint = (return_volatility * 0.4 + 
                      (volume_trend - 1) * 0.3 + 
                      (amount_trend - 1) * 0.3)
    ...
    async def test_signal_consistency(self):
        ...
        try:
            ...
            self.results['signal_consistency']['details'].append(f"信号一致性测试失败: {e}")
    ...
    async def test_temporal_consistency(self):
        ...
        for window in windows:
            z_score = await self.calculate_individual_z_score_with_window(stock_code, window)
    ...
    async def calculate_individual_z_score_with_window(self, stock_code: str, window: int) -> float:
        """计算指定窗口的个体Z分数"""
        self.cursor.execute("""
            SELECT pct_chg
            FROM daily_prices 
    ...
    async def test_universe_processing(self):
        ...
        try:
            ...
            self.results['universe_processing']['details'].append("宇宙处理策略测试通过")
            logger.info("宇宙处理策略测试通过")
            
        except Exception as e:
            self.results['universe_processing']['status'] = 'failed'
            self.results['universe_processing']['details'].append(f"宇宙处理策略测试失败: {e}")
    ...
    async def test_core_universe_processing(self):
        ...
        for stock_code in core_stocks[:10]:  # 测试前10只
            ...
            quant_fingerprint_z = await self.calculate_quant_fingerprint_z_score(stock_code)
            
            if not np.isnan(macro_risk_z) and not np.isnan(market_style_z) and not np.isnan(quant_fingerprint_z):
    ...
    async def generate_test_report(self):
        ...
        # 保存报告
        report_file = f"quantsignal_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        import json
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        
        logger.info(f"测试报告已保存: {report_file}")
        logger.info(f"QuantSignalEngine测试完成 - 通过率: {report['summary']['pass_rate'] * 100:.1f}%, 耗时: {total_duration:.2f}秒")
