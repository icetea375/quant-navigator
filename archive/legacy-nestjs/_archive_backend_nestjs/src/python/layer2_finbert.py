...
class Layer2FinBERT:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 情感标签映射
        self.label_map = {"negative": -1, "neutral": 0, "positive": 1}
        self.reverse_label_map = {v: k for k, v in self.label_map.items()}

    def load_pretrained_model(
        self, model_name: str = "yiyanghkust/finbert-tone"
    ) -> None:
        """加载预训练模型"""
        try:
    ...
    def train(
        self, texts: List[str], labels: List[int], validation_split: float = 0.2
    ) -> Dict[str, Any]:
        """训练模型"""
        try:
            ...
            val_true_labels = [
                dataset[i]["labels"].item() for i in range(len(val_dataset))
            ]
            ...
            self.logger.info(
                f"Training completed. Accuracy: {accuracy:.4f}, F1: {f1:.4f}"
            )
        ...
        try:
            ...
            with torch.no_grad():
                for i in range(0, len(texts), self.batch_size):
                    batch_input_ids = data["input_ids"][i : i + self.batch_size].to(
                        self.device
                    )
    ...
    def evaluate(self, texts: List[str], labels: List[int]) -> Dict[str, float]:
        ...
        try:
            ...
            pred_labels = np.where(
                predictions > 0.2, 1, np.where(predictions < -0.2, -1, 0)
            )
