...
class ArbitrationService:
    ...
    async def get_cases(
        ...
        return {"data": paginated_cases, "total": total}
    ...
    async def create_case(self, case_data: ArbitrationCaseCreate) -> ArbitrationCase:
        ...
        disagreement_score = self._calculate_disagreement_score(
            case_data.qwen_analysis, case_data.doubao_analysis
        )
    ...
    async def preprocess_case(self, case_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "disagreement_analysis": {
                ...
                "description": self._get_disagreement_description(
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        pending_cases = len(
            [case for case in self.cases.values() if case.status == "pending"]
        )
        resolved_cases = len(
            [case for case in self.cases.values() if case.status == "resolved"]
        )

        # 计算平均分歧度
        disagreement_scores = [case.disagreement_score for case in self.cases.values()]
        avg_disagreement = (
            sum(disagreement_scores) / len(disagreement_scores)
        ...
        return {
            ...
            "high_disagreement_cases": len(
                [score for score in disagreement_scores if score > 0.7]
            ),
        }
    ...
    def _generate_case_summary(self, case: ArbitrationCase) -> str:
        ...
        if case.doubao_analysis:
            summary_parts.append(
                f"豆包情感: {case.doubao_analysis.sentiment} (得分: {case.doubao_analysis.score:.2f})"
            )
