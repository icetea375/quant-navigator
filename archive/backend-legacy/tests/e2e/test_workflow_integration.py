...
class TestWorkflowIntegration:
    ...
    @pytest.mark.asyncio
    async def test_daily_workflow_complete_flow(
        self, workflow_service, arbitration_service, report_service, llm_service
    ):
        """测试日常分析工作流完整流程"""
        # Arrange - 准备测试数据
    ...
    @pytest.mark.asyncio
    async def test_arbitration_case_creation_and_processing(
        self, arbitration_service, llm_service
    ):
        """测试仲裁案件创建和处理流程"""
        # Arrange - 准备测试数据
        case_data = ArbitrationCaseCreate(
            ...
            doubao_analysis=SentimentAnalysis(
                sentiment="positive", score=0.7, reasoning="市场情绪积极"
            ),
        )
        ...
        # Act - 预处理案件
        preprocess_result = await arbitration_service.preprocess_case(
    ...
    async def test_report_creation_and_management(self, report_service):
        ...
        # Act - 查询报告
        retrieved_report = await report_service.get_report_by_id(
