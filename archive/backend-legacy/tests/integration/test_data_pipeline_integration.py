...
class TestDataPipelineIntegration:
    ...
    async def test_should_extract_financial_factors_from_tushare_data_when_valid_data_provided(
        ...
        # Assert - 验证提取的财务因子
        assert financial_factors is not None, "财务因子提取结果不应为空"
        assert (
    ...
    async def test_should_calculate_quant_signals_when_financial_factors_provided(
        ...
        # Assert - 验证量化信号计算结果
        assert quant_signal is not None, "量化信号计算结果不应为空"
    ...
    async def test_should_detect_anomalies_when_abnormal_data_provided(
        ...
        # Assert - 验证异常检测结果
        assert anomalies is not None, "异常检测结果不应为空"
        assert len(anomalies) > 0, "应该检测到至少一个异常事件"
    ...
    async def test_should_persist_quant_signals_to_database_when_valid_signals_generated(
        ...
        # Assert - 验证信号已保存到数据库
        assert saved_signal is not None, "保存的量化信号不应为空"
        assert saved_signal.signal_id is not None, "保存的信号应有ID"
        assert (
        ...
        assert retrieved_signal is not None, "应该能从数据库查询到保存的信号"
        assert (
