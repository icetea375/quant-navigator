...
import { List, Card, Tag, Button, Input, Select, Typography, Space, Tooltip, Badge, Row, Col, Statistic, Divider } from 'antd';
import {
  SearchOutlined,
  FilterOutlined,
  ClockCircleOutlined,
...
const ArbitrationCaseList: React.FC<ArbitrationCaseListProps> = ({
  ...
  const filteredCases = useMemo(() => {
    ...
    if (searchText) {
      filtered = filtered.filter(caseItem =>
        caseItem.stockCode.toLowerCase().includes(searchText.toLowerCase()) ||
        caseItem.stockName.toLowerCase().includes(searchText.toLowerCase()) ||
        caseItem.reportType.toLowerCase().includes(searchText.toLowerCase())
      );
    }

    // 优先级过滤
    if (priorityFilter.length > 0) {
      filtered = filtered.filter(caseItem => priorityFilter.includes(caseItem.priority));
    }

    // 状态过滤
    if (statusFilter.length > 0) {
      filtered = filtered.filter(caseItem => statusFilter.includes(caseItem.status));
    }
  ...
  const renderStatistics = () => {
    ...
    const inProgress = cases.filter(caseItem => caseItem.status === 'in_progress').length;
    const completed = cases.filter(caseItem => caseItem.status === 'completed').length;
    const urgent = cases.filter(caseItem => caseItem.priority <= 2).length;
