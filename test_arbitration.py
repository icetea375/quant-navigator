...
async def test_database_connection():
    ...
    try:
        # 连接数据库
        conn = await asyncpg.connect("postgresql://postgres:password@localhost:5432/quant_navigator")
        print("✅ 数据库连接成功")
        
        # 查询仲裁案件
        rows = await conn.fetch("SELECT case_id, stock_code, status FROM arbitration_cases LIMIT 5")
