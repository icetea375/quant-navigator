...
class PredictionGenerator:
    ...
    def _load_prompt_templates(self) -> Dict[str, Any]:
        """加载预测生成的Prompt模板"""
        try:
            template_path = Path("config/prompt_templates/prediction_generation_prompts.json")
        ...
        return {
            ...
            "generate_probabilistic_forecast": {
                ...
                "output_format_definition": {
                    "scenario_optimistic": { "return": "<float>", "reasoning": "<string>" },
                    "scenario_neutral": { "return": "<float>", "reasoning": "<string>" },
                    "scenario_pessimistic": { "return": "<float>", "reasoning": "<string>" },
                    "probabilities": { "optimistic": "<float>", "neutral": "<float>", "pessimistic": "<float>" },
                    "key_drivers": { "positive": ["<string>"], "negative": ["<string>"] }
                }
            }
        }
    
    def predict(self, stock_code: str, trade_date: str, event_chain: List[Dict[str, Any]], 
                mda_scores: Dict[str, Any]) -> Dict[str, Any]:
        """
        生成多场景概率性预测
    ...
    def _prepare_decision_package(self, stock_code: str, trade_date: str, 
    ...
    def _get_sentiment_analysis(self, stock_code: str, trade_date: str) -> Dict[str, Any]:
        """获取情感分析"""
        # 这里应该从数据库查询情感分析结果
        # 暂时返回模拟数据
        return {
...
请严格按照上述JSON格式输出，不要包含任何其他内容。"""
    ...
    def _validate_prediction(self, prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 验证必要字段
            required_fields = ["scenario_optimistic", "scenario_neutral", "scenario_pessimistic", "probabilities"]
    ...
    def _calculate_overall_score(self, prediction: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # 概率分布合理性评分
            probabilities = prediction.get("probabilities", {})
            prob_entropy = -sum(p * (p if p > 0 else 0.001) for p in probabilities.values())
            ...
            return_range = max(s.get("return", 0) for s in scenarios) - min(s.get("return", 0) for s in scenarios)
            diversity_score = min(return_range / 20, 1.0) * 0.2  # 20%的收益率差异为满分
            
            # 驱动因素完整性评分
            key_drivers = prediction.get("key_drivers", {})
            driver_count = len(key_drivers.get("positive", [])) + len(key_drivers.get("negative", []))
    ...
    def _create_empty_prediction(self, stock_code: str, trade_date: str) -> Dict[str, Any]:
        """创建空预测"""
        return {
            "stock_code": stock_code,
            "trade_date": trade_date,
    ...
    def get_prediction_summary(self, prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算期望收益率
            expected_return = (
                scenarios.get("scenario_optimistic", {}).get("return", 0) * probabilities.get("optimistic", 0) +
                scenarios.get("scenario_neutral", {}).get("return", 0) * probabilities.get("neutral", 0) +
                scenarios.get("scenario_pessimistic", {}).get("return", 0) * probabilities.get("pessimistic", 0)
            )
            ...
            variance = sum(p * (r - expected_return) ** 2 for r, p in zip(returns, probabilities.values()))
