...
class EventChainBuilder:
    ...
    def _load_prompt_templates(self) -> Dict[str, Any]:
        """加载事件链构建的Prompt模板"""
        try:
            template_path = Path("config/prompt_templates/event_chain_building_prompts.json")
    ...
    def build(self, stock_code: str, trade_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 构建事件链
            event_chain = self._build_event_chain(events, mda_features, stock_code, trade_date)
    ...
    def _get_anomaly_events(self, stock_code: str, trade_date: str) -> List[Dict[str, Any]]:
        """获取异常事件列表"""
        # 这里应该从数据库查询异常事件
        # 暂时返回模拟数据
        return [
    ...
    def _build_event_chain(self, events: List[Dict[str, Any]], mda_features: Dict[str, Any], 
                          stock_code: str, trade_date: str) -> List[Dict[str, Any]]:
        """使用LLM构建事件链"""
        try:
            # 准备输入数据
...
请严格按照JSON格式输出，不要包含任何其他内容。"""
    ...
    def _calculate_chain_quality(self, event_chain: List[Dict[str, Any]]) -> float:
        ...
        # 因果关系评分
        causal_links = sum(1 for event in event_chain if '因果' in event.get('event_description', ''))
        causal_score = min(causal_links / len(event_chain), 1.0) * 0.2
        
        # 战略关联评分
        strategy_links = sum(1 for event in event_chain if event.get('mda_strategy_link'))
    ...
    def get_chain_summary(self, event_chain: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        causal_events = sum(1 for event in event_chain if '因果' in event.get('event_description', ''))
        strategy_events = sum(1 for event in event_chain if event.get('mda_strategy_link'))
